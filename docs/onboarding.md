...
- PNR data: `PNR_SOURCE=railkit` and `RAILKIT_API_KEY`. `PNR_FALLBACK` stays `none` — RailKit is the only provider, so nothing else names anything. A deployment still carrying the retired `rapidapi` is read as `none` and warned once at boot rather than refused; **delete that variable** and the accommodation in `env.ts` can go too. It is therefore a single point of failure: watch the plan's renewal date, because an expired plan quietly drops to 50 requests a month, and run `npm run source:health` to find out before travellers do.
