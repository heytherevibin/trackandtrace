...
- **Grants are explicit.** Every function: `revoke all on function … from public, anon, authenticated;` then a single `grant execute` to the role that may call it. Every table: no grants to `anon` or `authenticated` at all.
