...
Next.js server
  ...
  console frame (src/console/components/console-frame.tsx) — every signed-in page's shell: an environment strip, ConsoleMasthead (clock, MemberMenu), the module rail (ConsoleRail on sm and up, ConsoleRailDrawer's left drawer below it — both always mounted, CSS picks one), then the page. The rail draws nothing yet: `railFor()` (src/console/nav.ts) hides a module until its own `built` flag flips true in the PR that ships its page, so a role with no built module sees no rail and no drawer trigger at all rather than fourteen dead links (2d-1 ships none; 2d-2 ships Team and Audit log; 2f ships Overview). `/` redirects to `/keys` — My keys (src/app/console/keys/page.tsx) — the one page this phase builds: the Keys plate (list, add, rename, remove), the Profile plate, and the Sessions plate (this device, sign out others), fed by one `console_my_keys` call plus `console_my_sessions`.
