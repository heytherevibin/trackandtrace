  ...
  return (
    <ul aria-label={messages.shell.footer.sections} className="mt-3.5 flex list-none flex-col gap-2.5 p-0">
      {LANDING_SECTIONS.map(({ id, label, Icon }) => (
        <li key={id}>
          <a href={`#${id}`} className="inline-flex items-center gap-2 font-display text-label font-semibold uppercase tracking-caps text-accent-text no-underline hover:text-accent-soft-ink">
