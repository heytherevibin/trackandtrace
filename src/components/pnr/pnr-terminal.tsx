...
function useCheckPlate(sampleMode: boolean) {
  ...
  const run = async () => {
    ...
    const view = terminalResult(outcome, { pnr, attemptedAt, sampleMode });
...
export function PnrTerminal({ sampleMode }: { readonly sampleMode: boolean }) {
  const plate = useCheckPlate(sampleMode);
...
export function PnrClosingTerminal({ sampleMode, title, meta, lead }: { readonly sampleMode: boolean; readonly title: string; readonly meta: string; readonly lead: string }) {
  const plate = useCheckPlate(sampleMode);
