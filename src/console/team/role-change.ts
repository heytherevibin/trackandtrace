...
// The two facts a role change needs before it can be offered, decided in the browser from the
// roster the Team page already holds (task-5-addendum.md §3).
//
// Deciding them here is not a shortcut past the database. `console_change_role` re-checks both
// itself and refuses regardless -- that is the security boundary, and this file is not. What this
// file buys is the sheet's own dlg_owner ("A console needs at least one Owner" / "Make someone
// else Owner first.") instead of a refusal nobody can read: the database raises one developer
// string, 'a console needs at least one owner', for two different cases, with SQLSTATE 42501 --
// the very same code every other console refusal raises, down to console.use_tap's "no tap for
// this action". There is nothing in that error to tell the cases apart by, and reading its *words*
// to find out is what confirm-its-you.tsx's own note rules out: it breaks silently the first time
// either string is edited, which is the moment it was supposed to help.
 ...
 *
 * Where the two ever disagree, the migration is right and this is the thing that is wrong.
 */
export function needsAnotherOwner(target: RoleChangeTarget, next: ConsoleRole, roster: RoleChangeRoster): boolean {
