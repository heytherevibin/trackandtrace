 ...
 *
 * A prefetch is excluded, which the ruling did not anticipate and this page has to: once module 14
 * is `built`, the rail draws a link to it on every console page, and Next prefetches links it can
 * see. A prefetch is not someone opening the log, and an append-only record of something that did
 * not happen cannot be taken back. `next-router-prefetch` is the header Next sends for exactly that
 * request, and a missing header falls through to writing the row -- the safe direction, since the
 * cost of a false negative is one unrecorded open and the cost of a false positive is a permanent
 * untrue one.
 *
