...
describe("a cursor reset", () => {
  ...
  it("does NOT name a combo with no cursor entry at all, and such a run IS whole", async () => {
    // A lost cursor file and a first run are the same input — an empty map — and nothing here can
    // tell them apart, so neither is called a restart. `crawl-availability.mjs`'s header used to
    // claim a lost file made the run un-whole; it never did, and this pins which of the two is true.
    // Only a cursor in the PAST names a band, because only then is there a band to name.
    const { ask } = stubAsk(() => OK);
    const summary = await run({ ask, cursors: {} });
